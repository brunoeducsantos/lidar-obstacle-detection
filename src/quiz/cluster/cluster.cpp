...
void proximity(int indice, std::vector<int> &cluster, std::vector<std::vector<float>> points, KdTree *tree,  float distanceTol, std::vector<bool> &processed){
		processed[indice]= true;
		cluster.push_back(indice);
		std::vector<int> nearby_points= tree->search(points[indice],distanceTol);
		for (auto index: nearby_points){
			if(!processed[index]){
				proximity(index, cluster, points, tree, distanceTol, processed);
			}
		}
}
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for(int i=0;i<points.size();i++){
		...
		proximity(i, clust, points, tree,  distanceTol, processed);
